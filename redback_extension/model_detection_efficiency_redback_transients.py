...
# Cosmology (from TEGLON's Settings.ini) is loaded LAZILY in main() via _load_cosmology(), so
# this module imports with NO TEGLON dependency. `initial_z` reads these module globals, which
# are set before the multiprocessing Pool (inherited by fork).
H0 = O_M = O_DE = None


def _ensure_teglon_on_path(explicit_root=None):
    """Put the TEGLON app root (the directory that contains `web/src`) on sys.path, so this
    SEPARATE, drop-in extension can import TEGLON's internals WITHOUT living inside the package
    and WITHOUT modifying TEGLON. Resolution order:
      1. --teglon_root argument,
      2. $TEGLON_ROOT,
      3. already importable (e.g. the container's PYTHONPATH=/app),
      4. walk up from the cwd and from this file's directory.
    Returns the resolved root path; raises ImportError with guidance if not found.
    """
    marker = os.path.join("web", "src", "utilities", "Database_Helpers.py")

    def _ok(root):
        return root and os.path.exists(os.path.join(root, marker))
    ...
    for start in (os.getcwd(), os.path.dirname(os.path.abspath(__file__))):
        ...
        for _ in range(8):
            ...
            d = nd
    raise ImportError(
        "Could not locate the TEGLON app root (the directory containing web/src). Run inside the "
        "TEGLON container (which sets PYTHONPATH=/app), pass --teglon_root /path/to/teglon, or set "
        "the TEGLON_ROOT environment variable.")


def _load_cosmology(teglon_root):
    """Set module-global cosmology (H0, O_M, O_DE) from TEGLON's Settings.ini. Called in main()
    before the multiprocessing Pool so `initial_z` workers inherit them (Linux fork)."""
    global H0, O_M, O_DE
    cfg = RawConfigParser()
    cfg.read(os.path.join(teglon_root, "Settings.ini"))
    H0 = cfg.get("cosmology", "H0")
    O_M = cfg.get("cosmology", "OMEGA_M")
    O_DE = cfg.get("cosmology", "OMEGA_DE")

# --- redback -> TEGLON simulation constants (added for the redback-transients variant) ---
# TEGLON's 30-column light-curve schema: time + 29 photometric bands (order is load-bearing).
BAND_COLS = [
...
# TEGLON band -> redback/sncosmo bandpass (verified vs redback 1.17 filters.csv).
# UKIRT J/H/K proxied by 2MASS; Clear (unfiltered) proxied by the broadest band.
BAND_TO_SNCOSMO = {
...
def _require_redback():
    """Lazily import redback (an OPTIONAL TEGLON dependency) only when this extension runs.

    Core TEGLON does not depend on redback, so this module imports fine without it; only
    invoking the redback-transients analysis needs it. Raise a clear, actionable error --
    with a docker install example -- if it is missing, instead of a bare ModuleNotFoundError.
    """
    try:
        from redback.model_library import all_models_dict
        from redback.priors import get_priors
        return all_models_dict, get_priors
    except ImportError as exc:
        raise ImportError(
            "\n" + "=" * 78 + "\n"
            "The redback-transients extension requires the OPTIONAL `redback` package,\n"
            "which is not installed in this TEGLON environment. Core TEGLON does not\n"
            "depend on redback -- install it only to use this optional analysis.\n\n"
            "Install redback INTO the TEGLON image/container (it needs a C compiler for\n"
            "sncosmo). Example with this repo's docker compose (adjust -p / paths as needed):\n\n"
            "  # persistent: add `redback` to requirements.txt (or pyproject) and rebuild\n"
            "  docker compose -p teglon_install --env-file docker/.env \\\n"
            "      -f docker/docker-compose.yml build teglon_cli\n\n"
            "  # quick, into a running container (not persistent):\n"
            "  docker compose -p teglon_install --env-file docker/.env \\\n"
            "      -f docker/docker-compose.yml exec teglon_cli pip install redback\n"
            + "=" * 78
        ) from exc
...
class Teglon:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        # --- redback transient options (this variant simulates the model bank instead of
        #     reading it from disk; the free parameters are the redback model's own) ---
        parser.add_option('--model', default='two_component_kilonova_model', type="str",
                          help='redback model name, e.g. `three_component_kilonova_model`')

        parser.add_option('--n_sim', default="2000", type="int",
                          help='Number of samples to draw from the model default prior')

        parser.add_option('--analysis_detection', default="True", type="str",
                          help='"True"/"False": run the detection-efficiency analysis after '
                               'simulating+saving. Pass "False" to simulate + save only.')

        parser.add_option('--z_ref', default="1e-3", type="float",
                          help='Reference redshift for absolute-magnitude conversion')

        parser.add_option('--seed', default="0", type="int",
                          help='RNG seed for reproducible prior sampling')

        parser.add_option('--from_disk', default="", type="str",
                          help='Analysis-only: directory of a PRE-SIMULATED redback .dat bank. '
                               'Parsed generically (redback NOT required) -- runs in a pristine '
                               'TEGLON container. Skips the simulation step.')

        parser.add_option('--teglon_root', default="", type="str",
                          help='Path to the TEGLON app root (the dir containing web/src). Only '
                               'needed if it is not auto-resolvable via PYTHONPATH or $TEGLON_ROOT '
                               '(e.g. when running this extension from outside the container).')

        return (parser)

    def simulate_and_save(self):
        """Simulate `--n_sim` light curves from the redback `--model` (its default prior),
        save them as TEGLON-format .dat files, and build the in-memory `models` dict the
        detection pipeline consumes. Returns (models, available_model_bands, free_param_names).

        redback returns *apparent* AB mag; we evaluate at `--z_ref` and subtract the distance
        modulus -> rest-frame absolute mag (TEGLON applies the real distance/extinction later).
        """
        import inspect
        import logging
        for _noisy in ("redback", "bilby", "matplotlib"):
            logging.getLogger(_noisy).setLevel(logging.WARNING)
        all_models_dict, get_priors = _require_redback()   # optional dep; friendly error if missing
        from astropy.cosmology import Planck18

        name = self.options.model
        model = name if name in all_models_dict else (name + "_model")
        if model not in all_models_dict:
            raise Exception("Unknown redback model `%s`" % name)
        fn = all_models_dict[model]

        sig_params = [p.name for p in inspect.signature(fn).parameters.values()
                      if p.name != "time" and p.kind != p.VAR_KEYWORD]
        prior = get_priors(model=model)

        n = self.options.n_sim
        z_ref = self.options.z_ref
        np.random.seed(self.options.seed)
        samples = prior.sample(n)
        if "redshift" in sig_params:                       # pin redshift -> absolute mag
            samples["redshift"] = np.full(n, z_ref)
        free_param_names = [p for p in sig_params if p in samples]

        mu = float(Planck18.distmod(z_ref).value)
        times = np.logspace(np.log10(1e-3), np.log10(150.0), 401)   # TEGLON KNe time grid
        t_clip = np.clip(times, 1e-3, None)

        out_dir = self.options.model_base_path
        if "{MODEL_TYPE}" in out_dir:
            out_dir = out_dir.replace("{MODEL_TYPE}", model)
        os.makedirs(out_dir, exist_ok=True)
        prefix = model[:-6] if model.endswith("_model") else model

        print("\n[redback] model=%s  n_sim=%s  z_ref=%s  mu=%.3f" % (model, n, z_ref, mu))
        print("[redback] free parameters: %s" % free_param_names)
        print("[redback] saving .dat bank to: %s" % out_dir)

        models = {}
        available_model_bands = list(ALL_COLS)
        for i in range(n):
            params = {k: float(samples[k][i]) for k in free_param_names}

            band_arrays = {}
            for bcol in BAND_COLS:
                app = np.asarray(fn(t_clip, output_format="magnitude",
                                    bands=[BAND_TO_SNCOSMO[bcol]], **params), dtype=float)
                absmag = app - mu
                # non-emitting / undefined epochs -> faint sentinel (finite, undetectable)
                band_arrays[bcol] = np.where(np.isfinite(absmag), absmag, 99.0)

            self._write_teglon_dat(out_dir, prefix, i, times, band_arrays, model,
                                   free_param_names, params)

            # interpolation funcs, same convention as the disk loader (interp domain t <= 15 d)
            keep = ~(times > 15.0)
            model_time = times[keep]
            model_key = tuple(params[k] for k in free_param_names)
            models[model_key] = {"time": interp1d(model_time, model_time, fill_value="extrapolate")}
            for bcol in BAND_COLS:
                models[model_key][bcol] = interp1d(model_time, band_arrays[bcol][keep],
                                                   fill_value="extrapolate")
            if i % 50 == 0 or i == n - 1:
                print("  [redback %s/%s] simulated + saved" % (i + 1, n))

        return models, available_model_bands, free_param_names

    def _write_teglon_dat(self, out_dir, prefix, i, times, band_arrays, model,
                          free_param_names, params):
        """Write one TEGLON-format ECSV .dat (30 cols, redback-native comment), binned 50/dir."""
        sub_dir = "%s/%s" % (out_dir, i // 50 + 1)
        os.makedirs(sub_dir, exist_ok=True)
        data = [np.asarray(times, dtype=float)] + \
               [np.asarray(band_arrays[b], dtype=float) for b in BAND_COLS]
        tab = Table(data, names=ALL_COLS)
        tab.meta["comment"] = (["type = %s" % model] +
                               ["%s = %s" % (k, params[k]) for k in free_param_names])
        tab.write("%s/%s_%05d.dat" % (sub_dir, prefix, i), overwrite=True, format="ascii.ecsv")

    def load_from_disk(self):
        """Analysis-only: load a PRE-SIMULATED redback .dat bank from disk WITHOUT importing
        redback. Parses the redback-native comment (`type = <model>` then `key = value` free
        parameters) generically into free_param_names + model_key, so the detection can run in a
        pristine TEGLON container that has no redback. Returns
        (models, available_model_bands, free_param_names)."""
        src = self.options.from_disk
        if "{MODEL_TYPE}" in src:
            src = src.replace("{MODEL_TYPE}", self.options.model)
        dat_files = sorted(glob.glob(os.path.join(src, "**", "*.dat"), recursive=True))
        if not dat_files:
            raise Exception("No .dat files found under `%s`" % src)
        print("\n[from-disk] loading %s model .dat files from %s" % (len(dat_files), src))

        models = {}
        free_param_names = None
        available_model_bands = None
        for mf in dat_files:
            model_table = Table.read(mf, format='ascii.ecsv')
            keys, vals = [], []
            for item in model_table.meta['comment']:
                if "=" not in item:
                    continue
                k, v = item.split("=", 1)
                if k.strip() == "type":
            ...
            if free_param_names is None:
                free_param_names = keys
            model_key = tuple(vals)

            # same masking convention as the original disk loader (NaN composite + t > 15 d)
            mt = Table(model_table, masked=True)
            for col in mt.columns:
                mt.mask[np.where(np.isnan(mt[col]))] = True
            mt.mask[np.where(mt['time'] > 15.0)] = True
            composite_mask = mt['time'].mask
            model_time = np.asarray(mt['time'][~composite_mask])

            models[model_key] = {}
            for col in mt.columns:
                models[model_key][col] = interp1d(
                    model_time, np.asarray(mt[col][~composite_mask]), fill_value="extrapolate")
            available_model_bands = list(mt.columns)
        print("[from-disk] free parameters: %s" % free_param_names)
        return models, available_model_bands, free_param_names

    def main(self):

        # region Build model bank: simulate via redback, OR load a pre-simulated bank from disk
        if self.options.from_disk:
            # analysis-only path -- redback is NOT imported; runs in a pristine TEGLON container
            models, available_model_bands, free_param_names = self.load_from_disk()
        else:
            models, available_model_bands, free_param_names = self.simulate_and_save()
            run_analysis = str(self.options.analysis_detection).strip().lower() in ("true", "1", "yes")
            if not run_analysis:
                print("\n--analysis_detection=%s -> simulation + save only; skipping detection."
                      % self.options.analysis_detection)
                return 0
        # endregion

        # region Bootstrap TEGLON (lazy, root-resolved -- keeps this extension separable)
        # The detection analysis reuses TEGLON's DB helpers + geometry classes. Import them here
        # (not at module top) so this file is a drop-in add-on that lives OUTSIDE the package and
        # needs no TEGLON modification. Simulation-only / --from_disk parsing already ran above.
        teglon_root = _ensure_teglon_on_path(self.options.teglon_root)
        ...
        _load_cosmology(teglon_root)
        print("[teglon] app root resolved to: %s" % teglon_root)
        # endregion
        ...
        # region Band abbreviation -> DB band-name mapping
        # NOTE: `models`, `available_model_bands`, and `free_param_names` were produced by
        # simulate_and_save() at the top of main() -- the redback simulation replaces the
        # on-disk model bank, so there is no file-reading / comment-parsing here.
        band_mapping = {
            "sdss_u": "SDSS u",
            "sdss_g": "SDSS g",
            "sdss_r": "SDSS r",
        ...
        # Build ascii.ecsv output -- columns ARE the redback model's free parameters + Prob.
        # `model_param_tuple` keys are tuple(params[k] for k in free_param_names), so the row
        # order matches `free_param_names` exactly.
        model_slug = self.options.model[:-6] if self.options.model.endswith("_model") else self.options.model
        cols = list(free_param_names) + ['Prob']
        dtype = ['f8'] * len(free_param_names) + ['f8']
        output_filename = 'Detection_Redback_%s_%s.prob' % (model_slug, self.options.sub_dir)

        result_table = Table(dtype=dtype, names=cols)
        for model_param_tuple, prob in net_prob_by_model.items():
            result_table.add_row(list(model_param_tuple) + [prob])
